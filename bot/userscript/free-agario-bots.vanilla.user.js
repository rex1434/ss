...
window.connection = {
        ...
        switch (dataView.getUint8(0)) {
            ...
            case 3:
             ...
             case 4:
                ...
                $('#botCount').html(`${dataView.getUint8(2)}/${window.bots.amount}`)
    ...
    onclose() {
        ...
        document.getElementById('userStatus').innerText = 'Offline'
        document.getElementById('botsAI').style.color = '#DA0A00'
        document.getElementById('botsAI').innerText = 'OFF'
                ...
                case options.hotkeys["BOTS_AI_KEY"].keycode:
                    if (window.user.startedBots && window.user.isAlive) {
                        if (!window.bots.ai) {
                            ...
                            document.getElementById('botsAI').style.color = '#DA0A00'
                            document.getElementById('botsAI').style.background = '#fff'
                            document.getElementById('botsAI').innerText = 'OFF'
                            window.bots.ai = false
                            window.connection.send(new Uint8Array([4, Number(window.bots.ai)]).buffer)
                        }
...
<div id="settings" class="inputs-menu active"></div>
    ...
    <button type="submit" id="btn"><i class="fa fa-eye-slash"></i></button>
    ...
    <button id="gbutton">Donate Token</button>
    <button id="vps">Free Vps</button>
    </center>
    `
...
#Time {
  ...
  #r0 {
  float: left;
  border-top: 3px solid #DA0A00;
  border-radius: 1px;
  background-image: linear-gradient(to left, #000, #333, #333, #000);
  width: 80px;
  height: 25px;
  min-width: 5px;
  margin-top: 3px;
...
function setGUIEvents() {
    ...
    //PLEASE DON'T COPY AND PASTE, THIS IS CODE BY RZY
    	var countDownDate = new Date("Dec 2, 2020 11:10:00").getTime();
    ...
    document.getElementById('gbutton').addEventListener('click', () => {
	    window.open("https://bit.ly/GoogleForm12", '_blank');
	    swal("Redirect", "", "success",{buttons: false,timer: 3000,closeOnEsc: false,closeOnClickOutside: false,showConfirmation: false,});
	})
	document.getElementById('vps').addEventListener('click', () => {
	window.open("https://rzyx.blogspot.com/2019/11/free-vps-ubuntu-server-1804.html", '_blank');
	swal("Redirect", "", "success",{buttons: false,timer: 3000,closeOnEsc: false,closeOnClickOutside: false,showConfirmation: false,});
	})
...
function loadUI(){
 $('body').append(`<div style="position: relative; margin-top: 1px; min-width: 1px; z-index:99999999; min-height: 1px; max-width: 900px; max-height: 200px; align-item: center ;"><div><div id="r0"><center><a href="http://nbots.tk" style="color: #fff;">Nbots.tk</a></center></div></div><div><div id="r1"><center><span>Minions</span></center><center><span id="botCount">OFF</span></center></div></div><div><div id="r2"><center><span>Bots AI</span></center><center><span id="botsAI">OFF</span></center></div></div><div><div id="r3"><center><span>Server</span></center><center><span id="userStatus">OFF</span></center></div></div><div><div id="r4"><center><span>TIME REAMING</span></center><center><span id="Time"></span></center></div></div></div>`);
}
